...
func getListeningAddr() (addr []string, err error) {
	if i := global.Config.IP; len(i) > 0 {
		return []string{fmt.Sprintf("http://%s:%d", i, global.Config.Port)}, nil
	}
...
func main() {
	...
	startHTTPServer := func() {
		wg.Add(1)
		go func() {
			err := http.ListenAndServe(fmt.Sprintf("%s:%d", global.Config.IP, global.Config.Port), ran.Serve())
			if err != nil {
	...
	startTLSServer := func() {
		wg.Add(1)
		go func() {
			err := http.ListenAndServeTLS(fmt.Sprintf("%s:%d", global.Config.IP, global.Config.TLS.Port),
	...
	redirectToHTTPS := func() {
		wg.Add(1)
		go func() {
			err := http.ListenAndServe(fmt.Sprintf("%s:%d", global.Config.IP, global.Config.Port), ran.RedirectToHTTPS(global.Config.TLS.Port))
			if err != nil {
				global.Logger.Fatal(err)
			}
			wg.Done()
